...
export const aiController = {
  ...
  async generate(_req: NextRequest) {
    const { userId } = await requireAuth();
    // Live Gemini call — throttle so one user can't drain the shared free-tier quota.
    rateLimit(`ai:generate:${userId}`, 5, MINUTE);
  ...
  async parse(req: NextRequest) {
    const { userId } = await requireAuth();
    // Live Gemini call per request — throttle per user (quota + concurrency).
    rateLimit(`ai:parse:${userId}`, 15, MINUTE);
