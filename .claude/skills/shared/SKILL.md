...
## Layout
- `src/shared/schemas/*.schema.ts` — zod schemas per domain: `user transaction category budget goal recurringRule netWorth notification aiInsight`. Each exports `Create*Schema`, `Update*Schema`, filter schemas, and the inferred types (`export type Foo = z.infer<typeof FooSchema>`).
